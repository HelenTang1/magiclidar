...
def compute_iou(box1, box2):
    """Compute standard IoU for boxes in [xmin, ymin, xmax, ymax] format.

    ``rescale_bboxes`` converts the model and target boxes from normalized
    ``cxcywh`` to pixel-space ``xyxy`` before this function is called.  The
    original Talk2Event ``test.py`` interpreted those ``xyxy`` values as
    ``[x, y, w, h]``, which enlarged both boxes and inflated the reported IoU.
    """
...
class Tester:
    ...
    def test(self):
        ...
        with open(f"{self.config.output_dir}/{self.split_checkpoint}_correct_iou.pkl", 'wb') as f:
    ...
    def calculate_metric(self, output_list):
        ...
        with open(f"{self.config.output_dir}/{self.split_checkpoint}_correct_iou.json", 'w', encoding='utf-8') as f:
